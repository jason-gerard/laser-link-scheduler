...
from src.models import pointing_delay_pair_nodes
...
class LLSModel(BaseScheduler):
    ...
    def _solve(self):
        ...
        if self.use_convex_penalty:
            ...
            self.flow_model += pulp.lpSum(capacities.values()) + pulp.lpSum(
                single_hop_capacities.values()
            )
        ...
        for gs_node, capacity in single_hop_capacities.items():
            print(
                f"Setting up inflow and outflow capacity constraints for ground station node {gs_node}"
            )
        ...
        for k in range(len(matched_edges_by_k)):
            ...
            contacts = [
                contact
                for contact in self.teg.contacts[k]
                if (contact.tx_node, contact.rx_node) in matched_edges
                or (contact.rx_node, contact.tx_node) in matched_edges
            ]
