...
def merge_node_capacities(
    capacities: list[NodeCapacity], node_idx: int
) -> NodeCapacity:
    """
...
def compute_wasted_capacity(capacities: list[NodeCapacity]) -> float:
    ...
    wasted_capacities = [
        abs(capacity.capacity_in - capacity.capacity_out)
        for capacity in capacities
        if capacity.capacity_out != float("inf")
    ]
...
def compute_scheduled_delay(
        ...
        for tx_idx in orbiter_nodes:
            is_in_contact = (
                len(
                    [
                        rx_idx
                        for rx_idx in non_source_nodes
    ...
    avg_delay_by_node = [
        node_delay["total_delay"] / max(node_delay["num_contacts"], 1)
...
def compute_all_delays(
    ...
    if idx1_rx != -1 and idx2_rx != -1:
        ...
        node_pointing_delay1, node_pointing_delay2 = all_pointing_delay(
            np.array([idx1_coords, idx1_rx_coords, idx2_coords]),
            np.array([idx2_coords, idx2_rx_coords, idx1_coords]),
            slew_rate=slew_rate,
        )
    ...
    return (
        link_acq_delay,
        [node_pointing_delay1, idx1, idx2, idx1_rx],
        [node_pointing_delay2, idx2, idx1, idx2_rx],
    )
